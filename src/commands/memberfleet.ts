...
import {ChannelType, Client, CommandInteraction, EmbedBuilder, EmbedField,} from "discord.js";
...
async function findShips(member: string) {
    ...
    let ownedShips = ships.filter(
        (ship) => {
            try {
                var lowerCaseOwner = ship.owner?.toLowerCase();
                return lowerCaseOwner == member?.toLowerCase()
            } catch (e) {
                console.error(e)
            }
        }
